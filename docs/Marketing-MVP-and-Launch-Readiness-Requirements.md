...
4. **Operational readiness is the phase most likely to be skipped and the one that carries
   real liability.** Source A says so explicitly (§25 Phase 2 risk, R-03). *(As observed at the
   original 2026-08-09 audit: backup was misconfigured, restore had never been tested, and the
   service runner had never run. **UPDATED 2026-08-19 — all three are stale as current claims:**
   backup execution was fixed and closed 2026-08-13 (RDY-0080); restore was proven — 7 of 7
   acceptance legs met, including an authenticated login against a restored instance with a
   negative control — and closed 2026-08-16 (RDY-0082); the background-service runner has run
   repeatedly since, most recently re-confirmed 2026-08-19 — see RDY-0083, which stays open for
   a different, real reason: the trigger is tied to a live console session and does not survive
   logoff, an Owner-flagged fragility, not a "never ran" defect.)* Provenance of one
   composer-installed, gitignored module is unknown; the audited install
...
| **0023** | Clinical depth: ≥15 completed SOAP notes, ≥10 encounters with vitals | GTM DEM-003 | 18 SOAP notes, 12 vitals seeded (PB-058/059, 2026-08-14) — count criteria met. **Growth-chart criterion (no seeded patient under 21) resolved 2026-08-19**: `SYN-0013` converted to a paediatric DOB (age 16), Owner-authorised, `EV-044` §12. Live UI render of the growth chart under an authenticated session not yet confirmed — same tooling-classifier login block as RDY-0042/0043 | READY AFTER DATA — counts met, growth-chart criterion resolved, live UI confirmation outstanding | DATA | **P0** | G2 | Clinical Workflow Reviewer | 0021 | NOT READY — see `EV-044` §12 |
...
| 0049 | Replace the three Unix-only commands configured on a Windows host (`lpr`, `enscript`, `/usr/bin/file`) and the placeholder OFX bank IDs | Audit §19.6, OD-04, L-21 | **PARTIAL FIX (2026-08-19, PR-20):** all three Unix-only settings in `sites/default/config.php` are now `PHP_OS_FAMILY === 'Windows'`-gated — Unix/Linux hosts keep the historical values byte-for-byte (zero behavior change); Windows hosts get `OPENEMR_PRINT_COMMAND = 'print /d:PRN'` (upstream's own documented alternative, same file's pre-existing comment) and an honest empty string for `OPENEMR_HYLAFAX_ENSCRIPT`/`file_command_path` (no drop-in Windows lpr/enscript/`file` substitute exists; `file_command_path` reconfirmed via tree-wide `git grep` to have zero runtime consumers in this codebase version — PB-163 had flagged this as a possibility but left the tree-wide search unattempted as "too slow on this filesystem"). **Not addressed by this fix:** the two placeholder OFX bank IDs (`123456789`), and no live printer/fax hardware verification was performed or is possible on this host (none exists to test against) — `php -l` clean is the verification bar, per the patch record. | NOT READY — DEFECT | CONFIGURATION | P1 | G3 | DevOps / Infrastructure | 0001 | NOT READY |
...
| **0096** | Define support channels, published hours, response target, escalation path and training plan. **No uptime figure may be published — none has been measured** | GTM §15.3 *Support/Training*, §21, §24 | **Owner decision 2026-08-19 (given directly in conversation): Level 1 — Business Hours selected; response target of first response within 1 business day confirmed**, no longer a candidate — see `EV-096` addendum. **Reflected 2026-08-19 (PB-360)** in `EV-066-pack.md` §9 (scope template) and `EV-068-pilot-requirements.md` elements 5-7/§4 (pilot-agreement requirements pack); both keyword-scanned clean for uptime/availability figures | **CLOSED 2026-08-19** — all six elements defined and reflected in both named downstream documents; no uptime/performance figure anywhere; response target agreed by the team with current staffing | COMMERCIAL | **P0** | G3 G6 | Sales / Pilot Owner | 0064, 0084 | **CLOSED 2026-08-19 — see `EV-096`, `EV-066-pack.md` §9, `EV-068-pilot-requirements.md`, PB-360** |
...
| 0108 | Add `audit_events_lab-order` to `$GLOBALS_METADATA` so lab-order events can be audited at all | Audit PL-02, GAP-0070, EB-02 | **Fixed** — `audit_events_lab-order` entry added to `$GLOBALS_METADATA` (`library/globals.inc.php`), PR-21. `src/Common/Logging/EventAuditLogger.php` already read this key (line 77) and mapped `procedure_order`/`procedure_order_code` to it (line 195-196); the global's absence meant it always read `false` with no UI control to change it | P2 | — | READY — ENGINEERING (PR-21) | DEFERRED |
...
**Current state:** *(At the 2026-08-09 audit: five services defined, two nominally active — `Email_Service` 2-minute interval, `UUID_Service` 240-minute — both `next_run` stuck at 2021-01-18, proving the runner had never executed; no live trigger of any kind; the browser-driven REST path dead because `rest_api = 0`, the legacy AJAX entry point with no application caller, the CLI command present but no Windows scheduled task referencing OpenEMR.)* **STALE — corrected 2026-08-14 (PB-071):** a Windows Scheduled Task trigger was built, registered, and proven with a negative control (fires only when a service is actually due). Re-confirmed live 2026-08-19: both services active, `next_run` current. **Held open for a different, real reason (PB-181, AGENT-OPS, per Owner instruction):** the trigger runs only as the logged-on console user and does not survive logoff (`G:` is a per-session Google Drive mount, so a `SYSTEM` task cannot see the application) — tested and ruled out as fixable on this host.
**Gap:** the trigger's dependence on an active console session, not "silently does not run" — that finding is superseded.
**Why it blocks launch:** For the demo it is still a disclosure item if the session has lapsed since the last logon (diagnostics screen would show overdue services in that case — mention it before the prospect notices it, per §40 row 12). For a pilot it is a defect: this exact mechanism cannot be the pilot's trigger, since a pilot host will not have a permanently logged-on operator console.
**Required action:** For the pilot host specifically (not this demo host), select a trigger mechanism that survives logoff — a proper Windows Service, a `SYSTEM`-context scheduled task against a non-Drive-mounted deployment, or enabling `rest_api` (own side effects noted below, still applicable) are the live options; none has been decided.
**Note on the once-considered alternative:** enabling `rest_api` would restore the trigger via `main.php:270`, but it also opens 98 API routes, which contradicts the current "every external surface is off" posture and adds prerequisites (`site_addr_oath` is empty, a hard blocker for tokens).
**Acceptance criteria:** met for this demo host (`next_run` advances for both active services within one interval — proven; diagnostics screen shows no overdue active service when the console session is active — proven). **Not met for a pilot deployment:** the chosen mechanism must be one that survives logoff, and none has been selected or written into the deployment runbook (`EV-047`).
...
| 0099 | **MFA enforcement** | CAP-0218, L-03, PL-01, EB-01 | Development — add a `force_mfa` global and wire it into `main_screen.php`. Audit sizes it **Small** | **No** — but every security conversation must currently state "cannot be enforced" | Enterprise review | **RETIRED — SUPERSEDED (Owner ruling 2026-08-19)** — this exact core-global mechanism conflicts with Locked Decisions Q5's centralized-Keycloak MFA direction; superseded, not merely deferred. See `EV-092` §3.1 addendum | The honest statement (RDY-0057) remains the interim — and now permanent-for-this-phase — answer |
...
**The readiness position:** on the 2026-08-09 audited baseline, **our own** backup configuration
failed, our own service runner had never executed, and our own deployment was 373 commits behind.
**UPDATED 2026-08-19:** backup configuration was fixed and proven 2026-08-13 (RDY-0080); the
service runner was built and proven 2026-08-14, still ticking as of today (RDY-0083 — held open
for a console-session-survival limitation on a pilot host, not a "never ran" defect); the
deployment gap moved the other direction — **484 commits behind upstream/master as of today**,
though the upstream `rel-820` merge itself has since been executed (RDY-0045, merge commit
`8e0eaba90`, push to origin still pending human credentials). The underlying point this section
makes — that operational readiness is easy to under-invest in because it is invisible to
marketing — holds regardless of which items have since closed.
...
| `PILOT READY` | *(2026-08-13 snapshot: backup execution proven but policy and restore still open; service runner never executed; 418 behind and divergent.)* **UPDATED 2026-08-19:** restore is now also proven and closed (RDY-0082, 2026-08-16); the service runner runs and is proven on this host (RDY-0083, held open only for a pilot-host logoff-survival gap); the `rel-820` merge has executed (RDY-0045, push still pending). **Still genuinely open:** backup *policy* (0081, off-instance target/retention/encryption undefined); HTTP only, no TLS; hosting undecided; DB credential unrotated from the upstream default (RDY-0048); secrets file `skip-worktree`-masked, not a real control |
