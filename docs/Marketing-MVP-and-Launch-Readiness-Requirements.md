...
| RDY | Requirement | Source | Audited state | Status | Gap type | Pri | Blocks | Owner | Deps | Verdict |
|---|---|---|---|---|---|---|---|---|---|---|
| **0071** | Documented export procedure covering CSV report export, full database export, and document/file export | GTM Pillar 2, D-2, MC-08, O-2/O-3 | Procedure written and executed (PB-045); 7 of 9 reachable CSV reports now verified exporting cleanly (PB-208), 1 had a genuine code defect (`pat_ledger.php`). **`pat_ledger.php` defect FIXED in code** (`6dea39a28`, `6ce9979f5`, code-review verified, `EV-071` §3.5.1) — this cell previously said "unfixed," now stale corrected. 2 reports still empty on unrelated seed gaps. Neither the code fix's live HTTP round-trip nor the reviewer leg has been exercised — both need an authenticated login, blocked by the same tool-permission-classifier issue documented at PB-217/218 for RDY-0042/0043/0048 | NOT READY — reviewer confirmation and live verification outstanding; code defect fixed | DOCUMENTATION | **P0** | G3 G5 G6 | DevOps / Infrastructure | 0059 | NOT READY — see `EV-071` §5.2 |
