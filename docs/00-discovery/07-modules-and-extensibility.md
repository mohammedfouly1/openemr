...
- **Whether the 6 non-`claimrev-connect` in-tree `oe-module-*` are byte-identical to their upstream counterparts** — carried from Phase 2 UNKNOWN (`03-directory-map.md:182`). Blocks a clean fork-vs-upstream diff on the module code specifically. **Resolved 2026-08-19** — see `docs/discovery/openemr-decision-evidence/17-q36-module-byte-identity.md`: at blob-SHA granularity, 6 of 7 tracked modules are byte-identical to upstream stable (`v8_2_0`); the seventh (`oe-module-comlink-telehealth`) differs by exactly one test-bootstrap file, attributable to an upstream commit on the release branch, not a fork edit. Confidence: CONFIRMED — **zero fork-only modifications** to any custom module, consistent with the fork having zero commits of its own (`02-repository-baseline.md`).
