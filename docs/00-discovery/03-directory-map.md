...
- **Distance from `openemr/openemr` upstream** — carried from `01-repo-inventory.md`; no `upstream` remote configured, so we cannot tell whether the seven `oe-module-*` dirs in this fork are byte-identical to upstream or fork-modified. Requires product-owner approval to `git remote add upstream https://github.com/openemr/openemr.git && git fetch upstream`. **Resolved in a later audit pass** — see `docs/discovery/openemr-decision-evidence/02-repository-baseline.md`, `05-upstream-fork-drift.md`, and `17-q36-module-byte-identity.md`: the remote was added; the fork's HEAD is a strict ancestor of `upstream/master` (zero fork-only commits), and all 7 tracked custom modules were confirmed byte-identical to upstream at the blob-SHA level (one exception attributable to a release-branch-only file, not a fork edit).
