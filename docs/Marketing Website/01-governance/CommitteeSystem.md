...
**Runtime policy, Owner amendment, 2026-08-20:** no role is bound to a named model or model tier. Use an available authorised runtime, preserve role independence, and disclose the runtime/model identifier when the platform reports it.
...
### 1.1 The `Committee` command — mandatory execution contract

When the Owner writes the standalone instruction **`Committee`** — or the common spelling
**`Comittee`** — in a task request, it is an explicit command to convene this system. It is not a
request for one assistant to analyse the task while speaking in the committee's name.

The command has these non-optional effects:

1. **M0 starts first** and creates a dispatch ID and the initial record before substantive task work.
2. M0 classifies every requested item, selects the smallest sufficient firing set, and records every
   selected and idle agent with the reason.
3. M0 runs §6.3 preflight and records runtime assignment, runtime reporting status, source access, output path,
   and skill status **per selected agent**.
4. The selected agents are actually dispatched under §7.2. Shell commands, database queries, or work
   performed by the Orchestrator are evidence collection — **never a substitute for an agent dispatch**.
5. Every selected agent writes its own registered output. M0 records its recommendation or verdict;
   agents do not receive the Owner's decision authority.
6. M5 performs the required terminal claim/evidence review; M6 and M4 fire where the technical rules
   require them. M0 then produces the twelve-section decision pack and one final result.
7. If any mandatory agent cannot run, M0 records `HELD` or `FAILED`, the missing agent and reason.
   The Orchestrator must not silently complete that agent's work and report the committee as convened.

**Default mode:** `Committee` means **FULL ROUND**. SINGLE-AGENT mode is permitted only when the
Owner explicitly writes `Committee — single agent: M<n>` (or equivalent) and the requested output
does not require a decision. A request touching a customer-facing artefact, a decision, demo access,
security, privacy, reset/re-base, or a locked decision cannot downgrade itself to SINGLE-AGENT.
...
| # | Agent | Runtime | Type | Owns |
|---|---|---|---|---|
| **M0** | **Chair** | Available authorised runtime | Chair | Classification · gates · the record · evaluation |
| **M1** | Market & Competitor Intelligence | Available authorised runtime | Generate | Acquisition mechanics, SERP, competitor surfaces |
| **M2** | Strategy & Ideas | Available authorised runtime | Generate | Options, ranked cheapest-first; the reopening licence |
| **M3** | Messaging & Content | Available authorised runtime | Generate | Message hierarchy, page copy |
| **M4** | Website Architecture & Conversion | Available authorised runtime | Generate | IA extension, funnel, forms — **and adversary to M6** |
| **M5** | Claim Compliance & Evidence | Available authorised runtime | **Adversary — veto** | Every artefact, against §32 / GTM §14 / the audit |
| **M6** | Technical & Security Auditor | Available authorised runtime | **Adversary** | Stack, exposure, abuse, privacy, operations |
| **M7** | Arabic & Localisation | Available authorised runtime | Generate | The Arabic surface, in both directions |
...
- **M4 attacks M6.** M6's per-layer verdicts are the only generated-style output in the system with
  ...
  and runs independently from M6. When the runtime exposes comparable quality settings, M4 is not deliberately configured below M6.
...
| **R2** | **Independent → simultaneous.** Generators answer **without seeing any other generator's answer**; M0 releases all answers at once, verbatim | Correlated agents and models anchor hard on whatever they read first. Sequential exposure manufactures agreement that does not exist |
...
| **R7** | **Runtime disclosure and reviewer independence** (§5). Record the runtime identifier when reported; otherwise record `NOT REPORTED BY RUNTIME`. Never deliberately configure a reviewer below a generator when comparable settings exist | Independence and honest disclosure matter; a product name is not evidence of review quality |
...
## 5. Runtime-neutral agent policy

No committee role is authorised, rejected, upgraded or downgraded by a named model product.
Commercial model names and tier labels are not operating requirements. The Orchestrator uses an available,
authorised agent runtime that can perform the role and write the registered deliverable.

| Requirement | Rule |
|---|---|
| Runtime selection | Use an available authorised runtime. Record the identifier reported by the platform |
| Runtime does not report a model | Record **`NOT REPORTED BY RUNTIME`**. This is valid with disclosure and is not, by itself, a reason to quarantine or invalidate output |
| No invented identity | Never infer a model name from assignment text, agent self-description, price, speed or behaviour |
| Comparable quality settings exist | Do not deliberately configure M5, M6 or M4's attack below the generator or auditor they review |
| Comparable settings do not exist | Record **`PARITY NOT VERIFIABLE`**. Continue with disclosure; do not claim parity was proven |
| Independence | A reviewer did not generate the artefact it reviews. M5 generates no replacement copy; M4's attack is separate from M6's audit |
| Required runtime unavailable | Record `NO EXECUTOR` and hold the affected committee. The Orchestrator does not impersonate the missing agent |

> **R7.** Review validity rests on independent roles, adversarial instructions, evidence quality and
> complete records. A commercial model name or tier is neither necessary nor sufficient evidence that
> a review is strong.

### 5.1 Two-stage runtime record

Both stages remain mandatory, but they are disclosure controls rather than named-model gates.

```
D3a  PRE-FLIGHT   Record the selected agent runtime, available quality/reasoning
                  setting if exposed, role, source access and output path.
                  PASS means a real executor exists for every firing role.

D3b  POST-FLIGHT  Record the runtime/model identifier actually reported by the
                  platform, or NOT REPORTED BY RUNTIME. Record failures, retries,
                  substitutions and observed setting changes.

NOT REPORTED BY RUNTIME = VALID WITH DISCLOSURE.
PARITY NOT VERIFIABLE = VALID WITH DISCLOSURE.
Neither may be represented as independently model-verified or parity-proven.
```

Quarantine is required only when there is affirmative evidence that a mandatory role did not run,
the wrong role generated the output, independence was broken, an unauthorised runtime was used, or
a configured setting violated an available comparable-quality rule. Missing model telemetry alone
does not trigger quarantine.

M0 chairs and records. Its runtime is logged under the same disclosure rule but does not establish
the quality of the content reviews.

---

## 6. Dispatch

### 6.0 Mandatory start when `Committee` / `Comittee` is invoked

Before reading toward a solution, running a substantive query, editing a governed file, or proposing
an answer, the Orchestrator dispatches **M0** with the Owner's request verbatim. M0 immediately creates:

- `committee/M0-dispatch-log.md` — an append-only dispatch entry;
- `committee/M0-decision-pack-<task>.md` — sections 1–4 opened with `STATUS: IN PROGRESS`;
- a unique dispatch ID in the form `COM-YYYYMMDD-NNN`.

M0's opening record must contain:

| Required record | Content |
|---|---|
| Trigger | Exact Owner text containing `Committee` or `Comittee` |
| Task and mode | Verbatim task · FULL ROUND by default |
| Classification | Evidence-determined · constraint-determined · genuinely open, per item |
| Firing set | Every M0–M7 role marked `FIRE` or `IDLE`, with one reason each |
| Runtimes | Selected runtime/setting and D3a executor result for every firing agent |
| Skills | Skill requested · installed/available · audited · `USED` or `NOT USED` · version/source when used |
| Sources | Frozen annex paths, hashes and timestamps |
| Outputs | One unique file path per firing agent |

No content may be represented as committee output until this opening record exists. At closure, M0
appends reported runtime identifiers (or `NOT REPORTED BY RUNTIME`), actual skills used, dispatch times, failures/retries, every agent's
position or verdict, M5's result, unresolved Owner decisions, and the final status.

**Proof rule:** a committee convened only if the log, the selected-agent files, and the final pack
exist and agree on the dispatch ID. Conversation prose, shell history and edited governance files do
not prove dispatch. If the proof set is incomplete, the truthful status is `NOT DISPATCHED`, `HELD`,
or `FAILED` — never `COMMITTEE COMPLETE`.
...
| **Runtime** | Available authorised runtime per role; no named-model requirement |
...
run), **both runtime-record stages** (the later reviewer remains independent and every reported substitution or setting change is disclosed), and **evaluate** (short form). A single-agent run appends to
`committee/M0-dispatch-log.md`:

```
DATE · REQUESTED BY · AGENT · RUNTIME SELECTED · RUNTIME REPORTED
CLASSIFICATION (one line)
RUNTIME RECORD: D3a PASS/FAIL · D3b REPORTED/NOT REPORTED/POLICY BREACH
OUTPUT FILE
...
the environment "should" support an executor or a tool is not a pass.**

| Check | PASS condition | On failure |
|---|---|---|
| **Runtime availability** | A real authorised executor exists for every firing role | Select another authorised runtime, or hold and record `NO EXECUTOR` |
...
| Runtime / agent / skill availability | **Every dispatch** (§6.3) |
...
### 6.6 Scope, depth and hard output budgets

The committee is a decision instrument, not an unlimited research programme. M0 records the Owner's
exact questions and an explicit **`OUT OF SCOPE`** list before preflight. An adjacent issue may be
logged in one line; it is not investigated unless it is necessary to answer the question or it is a
decision-blocking risk.

#### Depth modes

| Mode | Use | Findings | Authority |
|---|---|---|---|
| **COMPACT** | Narrow check or follow-up | ≤5 per agent | Owner or M0 when no decision/security surface is involved |
| **STANDARD** | Default for `Committee` / `$comittee` | ≤10 per ordinary agent; ≤15 for M6 | Default |
| **DEEP** | Whole-programme or exhaustive audit | Set in a written budget | **Owner approval before dispatch** |

`FULL ROUND` describes who participates; it does not mean unlimited depth. Unless the Owner writes
`DEEP` and approves a budget, every full round is `STANDARD`.

#### STANDARD character budgets — hard caps

Characters are the enforceable unit because runtimes do not reliably expose token use. Word and
reported-token counts are recorded when available, but they do not replace the character cap.

| Output | Maximum characters |
|---|---:|
| M0 opening content (stages 1–4) | **8,000** |
| `M0-dispatch-log.md`, whole dispatch entry | **12,000** |
| Ordinary generator M1/M2/M3/M4/M7 | **20,000 each** |
| M6 technical/security audit | **30,000** |
| M4 attack on M6 | **12,000** |
| M5 terminal review | **15,000** |
| M0 final decision pack, all twelve sections | **20,000** |
| Frozen evidence annex | **30,000** |
| **Whole dispatch, all governed outputs** | **100,000** |

#### Budget increase — Owner gate

An agent that expects to exceed a cap **stops before exceeding it** and returns:

```
BUDGET INCREASE REQUEST
DISPATCH ID:
AGENT:
CURRENT LIMIT / CURRENT USE:
ADDITIONAL CHARACTERS REQUESTED:
WHY THE CURRENT BUDGET IS INSUFFICIENT:
NEW FINDINGS THAT CANNOT FIT:
WHAT WILL BE OMITTED IF DENIED:
MAIN FILE OR ANNEX:
```

Only the Owner approves an increase, in writing, with the exact added character allowance. Silence,
an agent's self-approval, or an Orchestrator inference is a denial. Until approval, the agent closes
within budget using priorities and `UNKNOWN / NOT REVIEWED`. An unapproved overage makes the output
`HELD`; it is not accepted into the final pack.

#### Controls that make the budget achievable

1. **Write evidence once.** M0 creates `committee/COM-<id>-evidence-index.md`. Later files cite the
   evidence ID; they do not copy the evidence, query, quotation, source summary or hash table.
2. **Freeze extracts, not corpora.** The annex contains only task-relevant extracts and hashes, capped
   at 30,000 characters. M5's canonical §32 exception remains, but M5 cites rows rather than copying them.
3. **Finding caps.** Ordinary agent ≤10; M6 ≤15; M4 attack ≤5 substantive challenges; M5 ≤10
   BLOCK/objection rows. Overflow is a one-line deferred register ordered P0 → P1 → P2.
4. **External-source cap.** Five external sources per agent. More sources or a new legal/technical
   workstream requires a scope-and-budget request unless a current source is inaccessible.
5. **No restatement.** Do not repeat briefing text, role descriptions, firing-set reasoning, signed
   decisions or another agent's findings. Cite their IDs.
6. **Delta reviews.** M4 attacks only M6's strongest errors. M5 reviews decision-feeding claims,
   BLOCKs, REQUIRED CHANGEs and unsupported facts; neither re-performs the whole audit.
7. **Compact pack.** M0's final pack gives the direct answer, required changes, live options,
   divergence, BLOCKs, Owner decisions and finding-ID references. It does not reproduce agent reasoning.
8. **Stop early.** A decision-blocking finding stops dependent analysis. Record what remains unknown;
   do not spend budget designing a path that cannot yet be authorised.
9. **Twelve sections may be short.** `N/A — reason` or a finding-ID reference satisfies a section;
   completeness never requires duplication.
10. **Record actual use.** Every output ends with character limit, characters used, word count,
    approved extension and deferred-finding count. M0 runs the project budget checker before `FINAL`.

#### Scope expansion gate

For work outside the recorded Owner questions, the agent stops and submits:

```
SCOPE EXPANSION REQUEST
NEW WORKSTREAM:
WHY IT IS NECESSARY TO THE OWNER'S QUESTION:
RISK IF DEFERRED:
AGENTS / SOURCES / ADDITIONAL CHARACTERS:
```

Only the Owner approves. Without approval, record it in the deferred register and continue within
the original scope.

#### Transition for open dispatch `COM-20260820-001`

M4 and M6 outputs already completed before this rule are grandfathered as evidence and are not
expanded. Remaining stages adopt compact closure immediately: M4 attack ≤12,000 characters; M5
≤15,000; M0 adds ≤12,000 closure characters and must reference existing finding IDs. Any increase
requires the Owner gate above. This transition does not retroactively label the completed files compliant.
...
Within the cap, full nine-field prose is required for BLOCK, OPEN and material REQUIRED CHANGE
findings. Routine PASS items may be one compact table row carrying ID, statement, basis and status.
This is compression, not a lower evidence standard.
...
including when the honest content is *"not applicable this task"*. Sections cite evidence and agent
finding IDs instead of copying them; `N/A — reason` is complete when a stage genuinely does not apply.
...
| 3 | **Preflight** — capability and runtime record D3a (§6.3, §5.1) | R7 | `4 · GATES` |
...
| 8 | **Post-flight runtime record D3b** — reported identifier or disclosed absence; quarantine only on a policy breach | R7 | `4 · GATES` |
...
**Recorded every time, in `4 · GATES`:** the runtime/model identifier reported for each agent or `NOT REPORTED BY RUNTIME`,
...
**Also recorded for every firing agent:** `DISPATCHED: YES/NO` · start/end time · output path ·
skill requested · skill available · skill audited · skill actually used (or `NONE`) · concise
position/verdict · acceptance result. M0 records Owner rulings separately; it never attributes an
Owner selection such as `STEP0-001` or `SEED-001` to an agent.
...
how it was reached — every dissent, every runtime identifier reported or not reported, and every step that failed.
...
| Failure | Control |
|---|---|
| Runtime substitution or missing telemetry | §5.1 two-stage record · explicit disclosure · quarantine only on an affirmative policy breach |
| Rubber-stamp review | R7 independence · M5 never generates · **M4 attacks M6** · seeded-defect testing |
...
- **Never dispatches before D3a confirms a real executor**, and records D3b before release. Missing model telemetry is disclosed; an affirmative policy breach is quarantined.
...
## 8.1 M1 · Market & Competitor Intelligence · *Generate*
...
## 8.2 M2 · Strategy & Ideas · *Generate*
...
## 8.3 M3 · Messaging & Content · *Generate*
...
## 8.4 M4 · Website Architecture & Conversion · *Generate, and adversary to M6*
...
## 8.5 M5 · Claim Compliance & Evidence · **ADVERSARY, HOLDS A VETO**
...
## 8.6 M6 · Technical & Security Auditor · **ADVERSARY**
...
## 8.7 M7 · Arabic & Localisation · *Generate*
...
**4 · No presenter. Status: RULED — `NOGO-001` (§13.4b), and mostly already solved.** §40's no-go
register assumes a human in the room, and every row needed re-classifying for an unaccompanied
visitor as **restrict the route** / **on-screen notice** / **accept and disclose**.

**Measured rather than assumed.** Both roles were logged in over HTTP and each no-go route requested
as an authenticated GET, with OpenEMR's own error log read back to confirm every denial was a real
ACL decision. Controls (messages 49 KB, demographics 147–159 KB) returned full screens on the same
sessions, so the harness distinguishes *reached* from *denied*.

**The result is that `STEP0-001` D-c already did this work.** Choosing Front Office + Physician and
never Administrator restricts **12 of the 14 rows** with no new UI at all — including the two the
challenge named. Module Manager returns a 14-byte denial stub, so the three-module auto-registration
**cannot be triggered**. And every billing surface is 403 for both roles, so the invoicing boundary
*"spoken before any billing screen appears"* has **no screen to precede**; §13.2's form field 4
already asks the question before login.

**Two things are genuinely left, and only one is in the register.** Row 9: the Physician role reaches
Custom Report (HTTP 200, 4,114 B), the live path to Arabic PDF output mPDF cannot shape — restricted
by `NOGO-001`. And, not a register row at all: **the denial UX is inconsistent**, five different
shapes including a 500. A presenter narrates past a blank page; a visitor alone concludes the
software is broken. Making a locked door look locked is the remaining work — one notice, not
fourteen.
...
- D3a confirms a real executor for every firing role; D3b records the reported runtime identifier or `NOT REPORTED BY RUNTIME`; no affirmative runtime-policy breach remains unresolved.
```
...
### 13.4b `NOGO-001` — §40 re-classified for an unaccompanied visitor

```
NOGO-001 — SELF-SERVE HANDLING OF THE NO-GO REGISTER    ID: NOGO-001    STATUS: RULED
Governs: Challenge 4 (§11.4). Depends on: STEP0-001 D-c (never Administrator)

ROW 9 — ARABIC PDF VIA CUSTOM REPORT (chosen):
[x] RESTRICT THE ROUTE, DEMO PHYSICIAN ROLE ONLY. Production-role defaults must
       not change. Record the Arabic PDF limitation. Reconsider route access
       when Arabic PDF shaping is fixed, or when a render/export permission
       split allows the on-screen report without the broken export.
[ ] Leave reachable with an on-screen notice.
[ ] Split render from export.

ROW 7 — THE INVOICING BOUNDARY (chosen):
[x] ACCEPT AND DISCLOSE. No billing surface is reachable by either role, so the
       boundary has no screen to precede. The site states explicitly that
       billing is out of scope for the evaluation, and why — turning an absence
       into a disclosed boundary rather than a gap the visitor discovers.

ROWS 1-6, 8, 10-14: ALREADY ENFORCED by STEP0-001 D-c. No new work.

DATE: 2026-08-20     OWNER: mohammedfouly@noursolution.com
ROUTE RECEIVED: Claude Code session, 2026-08-20 — put to the Owner with the
       route-probe evidence below.
```

**The §40 self-serve column, measured:**
...
**Method, so it can be re-run and disputed.** Each row is an authenticated HTTP GET as
`r.aldosari` (Front Office) and `y.alharbi` (Physician), redirects not followed, with
`C:/openemr-stack/logs/php_error.log` read back to confirm each denial was a logged ACL decision
rather than an incidental blank. Controls on the same sessions returned full screens. Group
membership was confirmed independently: `gacl_aro_groups` places `r.aldosari` in **Front Office** and
`y.alharbi` in **Physicians**; neither is in Administrators.

**Three limits, stated rather than buried.** *(i)* This is the **local** instance, not
`demo.skyeagle.uk` — the same caveat that governs `SEED-001`. *(ii)* Only GET was probed; POST
surfaces were not. *(iii)* The Front Office leg of a third probe covering the clinical journey failed
on a harness fault and was **not** re-derived; PB-425 and PB-442 are the evidence for that journey,
not this pass.

**The implementation is a cloned group, not an edited one.** The permission lives in a single ACL row
— verified: `acl_id 11`, return value `view`, carrying **only** `patients/pat_rep`, mapped to
**only** the `doc` group. Dropping it would be one line, but that row *is* the stock Physicians
group's permission, and the ruling forbids changing production defaults. So
`06-nogo001-demo-physician-role.php` creates a separate **Demo Physician** group mapped to the other
three ACL rows and moves only the demo account into it. `check` mode has been run and is clean;
**`apply` has not been executed against any database.**

**And it must be folded into the clean baseline.** A reset restores the baseline's ACL tables, so a
reset performed before this change is baked in silently re-grants Patient Reports — the same
regression shape PB-442 recorded for `facility.primary_business_entity`, and the same lesson as
`SEED-001`'s weekday correction.
...
*"find why this audit is wrong"*, independently from M6, and not deliberately below it when comparable runtime settings exist → M5 checks whether any
...
| Challenge 4 | **RULED `NOGO-001`.** 12 of 14 §40 rows already enforced by D-c; row 9 restricted (script prepared, `apply` untested); row 7 accepted and disclosed. Remaining: the consistent denial page and one line of site copy |
...
| **2** | ~~Named-model review floor~~ **SUPERSEDED 2026-08-20.** Runtime-neutral policy now requires independence, disclosure and no deliberate under-configuration when comparable settings exist | §5 |
...
| **5** | ~~M0 named-model fallback ladder~~ **SUPERSEDED 2026-08-20.** M0 uses an available authorised runtime and records what the platform reports | §5 |
