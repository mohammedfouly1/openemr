...
**What this file replaces.** `MarketingCommittee.md` §1–§12 — its charter *and* the two independent
...
**There is no marketing agent, skill or plugin installed in this project** (`MarketingSkills.md`
...
- The pages `MarketingWebsite.md` §13.6 rules must not exist, do not get designed. `LOCKED`
...
knowledge (`MarketingWebsite.md` §11.3). Stated plainly so the absence is not read as an omission.
...
| **OPEN — do not assume a resolution:** whether the *site* is branded Thiqa or SkyEagle is undecided. A deliverable depending on it states the assumption | `MarketingWebsite.md` §13.8 Q2 |
...
| **The one genuinely missing edge control: rate-limiting on the login endpoint.** OpenEMR's own per-IP brute-force protection fires *after* the request reaches Apache | `MarketingWebsite.md` §13.3 |
...
The seed dated all 37 non-recurring appointments to a single day, `2026-08-14`. Today-filtered
screens rendered empty and the Flow Board showed `Total patients: 0` — **observed on the LOCAL
instance, 2026-08-19** (`Marketing-MVP…md:3318`, PB-441).
That symptom was then fixed, **manually**: PB-454 shifted the 37 rows forward five days;
`SELECT COUNT(*) … = CURDATE()` returned **16**; SS-06 and SS-07 were captured live against the
re-anchored data (`Marketing-MVP…md:1143`, PB-461…467).
**The structural gap is unchanged, and it is the item:** the shift was a one-off `+5 DAY` UPDATE —
not a re-base to `CURDATE()`, and not on a schedule — so the anchor **decays one day per day**, and
had already decayed by 2026-08-20. §16.2's requirement (re-base on every reset) remains unwired.
**Two checks before anything is scheduled:** *(i)* re-verify on **`demo.skyeagle.uk`**, not locally;
*(ii)* confirm PB-454's statement did not damage `pc_startTime` — see §13.4.

**3 · One shared database.** The product is **not multi-tenant** (GAP-0043 / L-07). Visitor A's
edits degrade Visitor B's session. **The reset is proven** — two byte-identical repeat resets from
the v4 baseline (PB-424) — **but runs on demand, not on a schedule.** Challenges 2 and 3 are **one
job**: a reset that restores the frozen dates re-creates Challenge 2 every time it runs.
...
3. **Edge controls** — rate-limit login and credential-request endpoints; bot protection; bounded
   ...
   `MarketingWebsite.md` §13.3 already names as missing.)*
...
### 13.4 The re-base hazard — check the pattern before scheduling it

**PB-454 is the reference fix Challenge 2 points at. Verify it before automating it.** It ran:
...
**`pc_startTime` is a `TIME` column, not a datetime** — `sql/database.sql:8281`.
...
shift and show a populated calendar and a 16-row Flow Board. That is hard to reconcile with wholesale
corruption, so this is a **flagged hazard, not a confirmed defect.**
...
`max_t` above `23:59:59`, or a non-zero `null_times`, confirms the hazard. **The run that decides
anything is on `demo.skyeagle.uk`, not on a local instance.**
...
**Objective.** Audit the architecture advised in `MarketingWebsite.md` §13. Return **approve / deny /
...
| Task 1 | **UNBLOCKED** — step-0 ruled `STEP0-001` (Reading B, sequence CONFIRMED, 2026-08-20). Dispatch on a passing §6.3 preflight; Challenges 2 and 3 ship first |
...
**enacts no rule change**. It supersedes `MarketingCommittee.md` as the operating instrument; that
