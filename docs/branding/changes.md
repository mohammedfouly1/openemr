...
| BRAND | Action | Status | Mechanism | Artefact | Evidence |
|---|---|---|---|---|---|
| 076 | BUILD-SHARED-THEME | DONE | Webpack entry map restricted to Q77 set | `public/themes/*.css` | **18** compiled CSS files at the top level: the 14 theme outputs `style_{light,dark}`, `compact_style_{light,dark}`, `rtl_style_{light,dark}`, `rtl_compact_style_{light,dark}`, `style_pdf`, `rtl_style_pdf`, `tabs_style_{full,compact}`, `rtl_tabs_style_{full,compact}` + **4** non-theme shells (`style.css`, `directional.css`, `ajax_calendar_ie.css`, `jquery.autocomplete.css`), plus one `misc/` directory holding 10 more files (19 directory entries in total); zero `solar`/`manila`/`cobalt_blue`/`forest_green` files. *(Corrected 2026-08-24, S1-P2-07: this row said "17 … + 3 non-theme shells" while itself enumerating four shells. 14 + 4 = 18, which is what `docs/demo-deployment-readiness.md:1591` and the SCAN2C count already said.)* |
