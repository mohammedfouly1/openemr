...
| V-09 | Rebase dry-run against upstream `master` reports conflicts only in the 6 recorded core files | **NOT VERIFIED AS STATED — see Surprises** | `git merge-tree HEAD upstream/master` (git 2.55, index-free preview, no working-tree mutation) reports conflicts in 10 files/areas, **none of which are the 6 recorded WS-C patch files** (`admin.php`, `interface/globals.php`, `FhirMetaDataRestController.php`, `OAuth2AuthorizationListener.php`, `ProductRegistrationService.php`, `TelemetryService.php` — all conflict-free). The actual conflicts are: `composer.json` (touched by branding tooling deps — a real branding-adjacent conflict), plus `composer.lock`, `.phpstan/baseline/offsetAccess.notFound.php`, `.phpstan/fatal-baseline-caps.php`, `.phpstan/phpstan.github.neon`, `docker/production/docker-compose.yml`, 5 files under `interface/modules/custom_modules/oe-module-faxsms/`, and `tests/Tests/E2e/AaLoginTest.php` — all general upstream drift (this branch's base, `dad282635`, is well behind current `upstream/master`, `feaf85c14`) unrelated to the branding patches |
...
| D-8 | Writable execution-denied volume | **RE-OPENED 2026-08-10 (RB-04)** | The plan's note is correct about the *recommended* route and wrong about the *shipped* system. `TokenCssWriter` is wired into `BrandingMaterialiser` and writes `…/oe-module-thiqa-branding/public/branding/<site>/tokens-{light,dark}.css` on every materialisation — observed after a real run: both files present, 1,553 B / 1,522 B, 2026-08-10 18:50. The deployed image therefore still needs that path writable. Nothing serves those files (only `FilesystemStylesheetProbe` reads them), and they are written even with an empty overlay. Closing this needs a route (a)/(b) decision — see `docs/RebrandingBugs.md` RB-04 |
...
8. ~~**Live materialisation has genuinely never run, even once, against the only existing tenant.**~~
   ...
   **empty Tier-2 overlay** — `saas_branding_tokens_light` and `saas_branding_tokens_dark` are both `''` —
   ...
   The materialisation also produced the two unserved `tokens-{light,dark}.css` files that re-opened
   dependency **D-8** — see RB-04. So this run is genuinely good news *and* the thing that exposed a
   design inconsistency; both facts belong in the record.
