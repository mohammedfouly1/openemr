...
| **4 (addendum)** | **2026-08-19** | **Correction only, no source change.** The "**418 commits behind `upstream/master`**" figure (§1.1, §17, `B-26`, §25.A) is **retracted as a measurement error, not merely stale**: this fork is a `rel-820`-based line (§25.A's own words), so the correct upstream-currency comparison is against `upstream/rel-820`, not `upstream/master` — comparing against `master` mixes in the entire 8.3.0-dev development delta that a rel-branch fork was never supposed to track. As of 2026-08-17 `upstream/rel-820` was merged into this branch, leaving a **1-commit residual gap**, not 418. This does not change any P0/P1/P2 classification or resolution step tied to `B-26` (a patch-currency pipeline is still owed before this becomes a customer instance) — only the magnitude of the figure is corrected. Not swept through every citing location — treat any inline "418 commits behind upstream/master" text elsewhere in this file as superseded by this entry. | No |
