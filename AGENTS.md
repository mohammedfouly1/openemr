...
3. Run and record preflight before dispatching the selected agents, including selected runtime,
   runtime/model identifier reported by the platform or `NOT REPORTED BY RUNTIME`, source access,
   output paths, and each skill requested, available, audited, and actually used.
...
6. Verify that the log, agent outputs, and final pack carry the same dispatch ID before reporting
   completion.
7. Enforce the scope and character budgets in CommitteeSystem.md §6.6. An agent that reaches its
   limit stops and requests an explicit Owner increase; silence is denial. Do not dispatch or accept
   unapproved work beyond the limit.
8. Write evidence once and reference its ID. Do not duplicate evidence, briefing text, classifications,
   or agent findings in later files.
...
The default depth is `STANDARD`, not unlimited. `DEEP` requires Owner approval before dispatch.
M0 records the exact Owner questions and an `OUT OF SCOPE` list; scope expansion and budget
increases require approval during the run. Before `FINAL`, run
`.agents/skills/comittee/scripts/check_dispatch_budget.ps1`. A failed budget check without a recorded
Owner exception makes the status `HELD`, not complete.
